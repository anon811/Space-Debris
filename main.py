...
f1 = open('./data/frame1.txt', 'r')
f2 = open('./data/frame2.txt', 'r')
frame1 = f1.read()
frame2 = f2.read()
...
def get_stars(canvas, ROWS, COLS, min_stars=30, max_stars=70):
    coroutines = []
    for i in range(random.randint(min_stars, max_stars)):
...
def draw(canvas):
    ...
    coroutines.append(fire(canvas, start_row=canvas_height//2, start_column=canvas_width//2, rows_speed=-0.008))
